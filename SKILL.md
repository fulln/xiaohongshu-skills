...
1. **认证相关**（"登录 / 检查登录 / 怎么登小红书 / 扫码 / 退出账号 / 切换账号"）→ 执行 `xhs-auth` 技能。
2. **内容发布**（"发布笔记 / 发帖 / 发个图文 / 帮我传个视频 / 写长文 / 定时发布"）→ 执行 `xhs-publish` 技能。
3. **搜索发现**（"搜索关于... / 找一下 / 看看这篇写了什么 / 浏览首页推荐 / 查看某人的主页"）→ 执行 `xhs-explore` 技能。
4. **社交互动**（"点个赞 / 收藏一下 / 帮我评论 / 回复他"）→ 执行 `xhs-interact` 技能。
5. **复合运营**（"竞品分析 / 热点追踪 / 总结爆款规律 / 批量互动 / 找对标账号"）→ 执行 `xhs-content-ops` 技能。
...
| 命令 | 功能 |
|------|------|
| `cli.py publish` | 图文发布（仅使用用户已准备好的本地图片） |
...
本系统采用 **Extension Bridge 模式**与小红书交互，这极大降低了风控风险：
1. 你（Agent）执行 `python scripts/cli.py` 发出指令。
2. CLI 将指令发送给本地的 WebSocket 服务（`bridge_server.py`）。
3. 用户浏览器中安装的 **XHS Bridge 扩展**连接到该服务，接收指令。
4. 扩展在用户*真实*的标签页、*真实*的登录态下执行页面操作（点击、输入、抓取）。
5. 结果返回给 CLI，并输出给你。
