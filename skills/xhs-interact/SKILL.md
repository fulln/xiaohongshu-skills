...
- **有感而发的评论建议**：若用户要求评论但未给出具体文案，Agent 应先读取笔记详情，根据内容生成 2-3 条符合小红书风格的评论建议（口语化、带 Emoji），让用户挑选或确认。
- **回复评论**：当使用 `reply-comment` 时，如果你需要找到目标评论的 `comment_id`，请先使用 `get-feed-detail` 查找该笔记的评论区获取。
