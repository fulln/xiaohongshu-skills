...
description: |
  ...
  当用户要求搜索小红书、查看笔记详情、浏览首页、查看用户主页，或需要为后续互动/分析收集 feed_id 与 xsec_token 时触发。
version: 1.0.0
metadata:
  openclaw:
    requires:
...
- downstream 详情、互动、分析操作统一使用成对的 `feed_id` + `xsec_token`。
- 搜索结果或首页 Feed 返回里的 `id`，在后续引用时应明确记作 `feed_id`。
- 结果应结构化呈现，突出关键字段。
- CLI 输出为 JSON 格式。
...
> 后续如果要获取详情、评论、点赞、收藏或评论互动，应把这里的 `id` 作为 `feed_id` 使用，并和对应 `xsec_token` 一起传递。
...
| 参数 | 可选值 | 适用场景 |
|------|--------|----------|
| `--sort-by` | 综合、最新、最多点赞、最多评论、最多收藏 | 找优质内容用"最多点赞"，跟进实时消息用"最新" |
| `--note-type` | 不限、视频、图文 | 明确用户想看视频还是图文时使用 |
| `--publish-time` | 不限、一天内、一周内、半年内 | 搜索时效性强的话题时带上限制 |
| `--search-scope` | 不限、已看过、未看过、已关注 | 筛选自己阅读记录时使用 |
| `--location` | 不限、同城、附近 | 找本地推荐、探店时使用 |
...
> 这里的 `id` 在 downstream 一律按 `feed_id` 处理，必须和同条结果里的 `xsec_token` 配对保存。

### 获取笔记详情

从搜索结果或首页 Feed 中取 `feed_id` 和 `xsec_token`，获取完整内容：
...
> **评论加载策略说明**：
> - 默认只加载第一页评论，速度最快。
> - 如果用户明确要求"看全部评论"、"分析评论区趋势"，才加上 `--load-all-comments`。
> - 若笔记评论很多，加载全部会极慢且容易超时，务必搭配 `--max-comment-items` 限制数量。
> - 子评论默认关闭，除非用户要求看深层互动，才加 `--click-more-replies`。
...
1. **笔记列表**：每条笔记展示标题、作者、互动数据，并保留后续可用的 `feed_id` + `xsec_token`。
