...
| `--publish-time` | 不限、一天内、一周内、半年内 | 搜索时效性强的话题（如突发新闻、近期活动）时带上限制 |
...
> **评论加载策略说明**：
> - 默认只加载第一页评论（通常十几条），速度最快。
> - 如果用户明确要求"看全部评论"、"分析评论区趋势"，才加上 `--load-all-comments`。
> - 若笔记评论成千上万，加载全部会极慢且容易超时，务必搭配 `--max-comment-items 50` 限制数量。
> - 子评论（"展开更多回复"）默认关闭，除非用户要求看深层互动，才加 `--click-more-replies`。
