...
**报告模版（必须严格按此 Markdown 表格输出）：**

| 笔记标题 | 作者 | 赞/藏/评 | 选题方向 | 封面风格 | 正文结构拆解 |
|---|---|---|---|---|---|
| [标题1] | [作者1] | 100/50/10 | [例如：干货盘点] | [例如：大字报/拼图] | [例如：痛点引入 -> 3个方法 -> 引导讨论] |
...
目标：根据用户指定的宽泛意图（如"帮我给最近的 AI 绘图笔记点点赞留点评论"），自动挑选中等热度的笔记进行互动。这个过程必须受到用户的监督。

**步骤：**
1. 搜索相关笔记（通常 `--sort-by 最新` 比较容易获得真实互动反馈）。
2. 在结果中筛选出 2-3 篇合适的笔记。
3. **关键：请求用户批准** - 列出拟互动的笔记标题，并生成建议的评论（参考 xhs-interact 有感而发策略），询问："我计划对以下笔记进行点赞并发表评论，是否执行？"
4. 获得批准后，循环执行 `like-feed` 和 `post-comment`，并在每次操作间自动使用系统自带延时工具暂停。
